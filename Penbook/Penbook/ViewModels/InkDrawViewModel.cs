...
using Penbook.Services.Ink.UndoRedo;

namespace Penbook.ViewModels
{
    public class InkDrawViewModel : Observable
    {
        ...
        private ICommand loadPdfFileCommand;
        ...
        private async Task LoadPdfFileAsync()
        {
            ClearSelection();
            var fileLoaded = await _fileService?.LoadPdfAsync();
